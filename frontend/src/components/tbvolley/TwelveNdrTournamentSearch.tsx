...
import type { TwelveNdrGender, TwelveNdrSource } from "@backend/sources/tbvolley/TwelveNdr";

type TwelveNdrTournamentSearch = ReturnType<typeof searchDecoders.twelvendr>;
type TwelveNdrTournament = TwelveNdrTournamentSearch["tournaments"][number];
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      const searches = await Promise.all(searchGenders.map(async (gender) => {
        ...
        const payload = await readJsonResponse(response, searchDecoders.twelvendr, `Не удалось загрузить ${config.title}`);
        ...
        <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_300px]">
          <div className="space-y-3 p-4 md:p-5">
            ...
            <TournamentSearchForm
              query={{ label: "Турнир", placeholder: config.source === "twelvendrcsvp" ? "CSV, Lima, Futures..." : "Vienna, Baden, ÖVV...", value: query, onChange: setQuery }}
              fields={[{ type: "select", label: "Сезон", value: season, onChange: setSeason, options: buildYearOptions().map((value) => ({ value, label: value })) }]}
              loading={loading}
              onSubmit={onSubmit}
            />
          </div>
