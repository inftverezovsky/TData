...
import type {
  BeachVolleyRuGender, BeachVolleyRuTournamentKind } from "@backend/sources/tbvolley/beach.volley.ru";

type BeachVolleyRuTournamentSearch = ReturnType<typeof searchDecoders.beachvolleyru>;
type BeachVolleyRuTournament = BeachVolleyRuTournamentSearch["tournaments"][number];
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      const searches = await Promise.all(searchGenders.map(async (gender) => {
        ...
        const payload = await readJsonResponse(response, searchDecoders.beachvolleyru, "Не удалось загрузить календарь beach.volley.ru");
        ...
        <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="space-y-3 p-4 md:p-5">
            ...
            <TournamentSearchForm
              query={{ label: "Турнир", placeholder: "Тула, Зеленоградск, финал...", value: query, onChange: setQuery }}
