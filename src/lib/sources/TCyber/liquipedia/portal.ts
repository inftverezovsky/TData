...
import { getPortalCache } from "@/lib/db/db";
...
const PORTAL_UPCOMING_WINDOW_DAYS = Number(process.env.LIQUIPEDIA_PORTAL_UPCOMING_WINDOW_DAYS || 60);
...
export async function fetchDisciplinePortal(slug: string, force = false): Promise<DisciplinePortalData> {
  ...
  if (!force) {
    ...
    const { resetProxyCooldowns } = await import("@/lib/proxy/proxySelector");
