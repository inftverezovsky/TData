...
| `tline`                                     | Проверки волейбола, флорбола и хоккея: расписания, jobs, leases, сравнения и решения оператора |
| `monitoring`                                | Проверки парсеров, отчёты и уведомления об изменении состояния                                 |
| `consultant`                                | Приватный Telegram-консультант по разрешённой документации, лимиты DeepSeek и состояние worker |
...
- Парольная защита раздела ограничена верхнеуровневыми настройками API `/settings` и их конфигурационными endpoints.
- Рабочие разделы Results/KHL, TLine и Sandbox доступны без административной сессии.
- Публичные операции всё равно проверяют Origin и ожидаемый Content-Type; отсутствие парольной формы не отменяет целостность HTTP-запроса.
- Фактическая отправка данных во внешнюю систему требует отдельной административной сессии.
- Тесты явно проверяют обе стороны: доступность рабочих сценариев и защищённость настроек API и внешней отправки.
...
## Допы штрафного времени КХЛ

`backend/src/results/khl/penaltyExtras.ts` рассчитывает семь допов при чтении
протокола, включая ранее сохранённые матчи. Каждый доп имеет стабильный `code`,
результат и подтверждающие события. Расчёт ограничен P1–P3; минуты 1–5 означают
00:00–04:59, первый двухминутный штраф имеет длительность ровно 2 минуты.
Причины сопоставляются с 15 вариантами предоставленного справочника.
Одновременные удаления разных команд и несовместимые причины остаются
неоднозначными; неполные данные не превращаются в отрицательный результат.
Нормализатор v5 сохраняет полноту списка удалений и сверяет его с доступными
суммами штрафных минут команд, включая овертайм и дисциплинарные штрафы.

`KhlPenaltyExtraBinding` хранит отдельный постоянный Admin ID каждого допа для
всех матчей КХЛ. Настройка: КХЛ → Настройки → Допы. Подтверждённый ID неизменяем;
уникальный индекс и сериализуемая транзакция запрещают повторное назначение.
Результаты видны в статистике матча, внутри блока «Штрафы».
Пока внешний контракт отправки допов не задан, они не добавляются в Admin payload.
