...
Production обновляется автоматически после успешного push в `main`:
[цепочка выкладки, доступ, миграции и откат](AUTO_DEPLOY.md). Ниже сохранены
ручные инструменты и исторический вариант Portainer для отдельной настройки.
Они не заменяют текущий серверный профиль с четырьмя сервисами приложения.
...
## Исторический вариант: отдельный Portainer Stack from Git

Для текущего production используйте автоматизацию выше. Новый стенд требует
отдельного профиля и применения миграций перед запуском web: Docker CMD больше
