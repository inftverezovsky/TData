...
/**
 * Собрать страницы без бизнес-записей → проверить весь снимок → под блокировкой сверить свежесть и качество →
 * атомарно обновить метаданные, участников, матчи и статус импорта. Last-good кэш публикуется после commit.
 * Force меняет загрузку, сохраняя защиту старого расписания, состава и ручных привязок.
 */
export async function importTournamentRecursive(params: {
  ...
  // 1. Главная страница возвращает кандидатов и исходный снимок; турнир пока не создаётся и не изменяется.
  ...
  // 2. Подстраницы читаются пакетами. Любой сбой учитывается при проверке полноты всего снимка.
    ...
    for (let batchStart = 0; batchStart < subPages.length; batchStart += CONCURRENCY) {
      ...
      for (const result of results) {
        if (result.status === "fulfilled") {
          ...
          subPageFailures += 1;
          logApiError("[Importer] Failed to process sub-page", result.reason);
        }
      }
    }
  }
  ...
  const commitResult = await prisma.$transaction(async (tx) => {
      ...
      if (existingTournament) {
        // Advisory lock объединяет новые импорты; row lock согласует запись с остальными обновлениями турнира.
        await tx.$queryRaw`SELECT id FROM "Tournament" WHERE id = ${existingTournament.id} FOR UPDATE`;
        existingTournament = await tx.tournament.findUniqueOrThrow({ where: { id: existingTournament.id } });
      }
