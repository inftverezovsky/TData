...
export function khlSyncRunView(run: KhlSyncRun): KhlSyncRunView {
  const summary = run.summary as KhlResultsSyncSummary | null;
  return {
    ...
    // Старые записи могли содержать внешние сообщения: фильтруем и при чтении, не изменяя сохранённое свидетельство.
    summary: summary ? {
      ...summary,
      failures: (Array.isArray(summary.failures) ? summary.failures : []).slice(0, 200).map((failure) => ({
        ...failure, message: safeKhlSyncError(new Error(failure.message)),
      })),
    } : null,
    error: run.error ? safeKhlSyncError(new Error(run.error)) : null,
  };
}
