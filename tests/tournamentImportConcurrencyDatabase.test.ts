...
const RUN_DATABASE_TEST = process.env.RUN_TOURNAMENT_IMPORT_RACE_DB_TESTS === "1";

if (!RUN_DATABASE_TEST) {
  test("a concurrent older import cannot overwrite a newer commit", {
    skip: "Set RUN_TOURNAMENT_IMPORT_RACE_DB_TESTS=1 with an isolated TEST_DATABASE_URL.",
  }, () => {});
  test("a concurrent TeamMapping propagation keeps participant identity and platformId", {
    skip: "Set RUN_TOURNAMENT_IMPORT_RACE_DB_TESTS=1 with an isolated TEST_DATABASE_URL.",
  }, () => {});
} else {
  test("a concurrent older import cannot overwrite a newer commit", {
    ...
    const databaseUrl = requireIsolatedDatabaseUrl(process.env.TEST_DATABASE_URL);
    ...
    try {
      ...
      const newerCommit = runSerializableTournamentImport(async (tx) => {
      ...
      let olderAttempts = 0;
      const olderCommit = runSerializableTournamentImport(async (tx) => {
        olderAttempts += 1;
      ...
      assert.equal((await prisma.tournament.findUniqueOrThrow({ where: { id: tournament.id } })).lastImportId, newer.id);
  ...
  test("a concurrent TeamMapping propagation keeps participant identity and platformId", {
    ...
    try {
      ...
      const manualEdit = prisma.$transaction(async (tx) => {
      ...
      let attempts = 0;
      const refresh = runSerializableTournamentImport(async (tx) => {
        attempts += 1;
        const observed = attempts === 1
          ? observeParticipantRead(tx, firstRead.resolve)
          : tx;
...
function observeParticipantRead(tx: Prisma.TransactionClient, onRead: () => void) {
  return {
    $queryRaw: (...args: unknown[]) => (
      tx.$queryRaw as (...queryArgs: unknown[]) => Promise<unknown>
    ).call(tx, ...args),
    tournamentParticipant: {
      ...
      deleteMany: (args: Parameters<typeof tx.tournamentParticipant.deleteMany>[0]) => tx.tournamentParticipant.deleteMany(args),
    },
...
function requireIsolatedDatabaseUrl(value: string | undefined) {
  ...
  const isolatedDatabase = /^tdata_import_race_test(?:_|$)/u.test(databaseName)
  ...
  if (!["postgres:", "postgresql:"].includes(url.protocol) || !(loopback || optedInDocker) || !isolatedDatabase) {
    throw new Error("Import race regression requires an isolated test database; Docker host postgres needs explicit opt-in");
  }
  return value;
}
